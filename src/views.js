...
function profile(req, user, p, links, socials, viewer) {
  ...
  const report = viewer && viewer.id !== user.id ? `<form class="report" method="post" action="/report/${encodeURIComponent(user.username)}">${csrfField(req)}<select name="category"><option>spam</option><option>harassment</option><option>impersonation</option><option>other</option></select><input name="reason" maxlength="300" placeholder="report reason"><button>report</button></form>` : '';
  const style = `--accent:${esc(p.accent)};--text:${esc(p.text_color)};--opacity:${Number(p.card_opacity)/100};--blur:${Number(p.blur_amount)}px`;
  return page(req, user.username, `<main class="profile" ${bg} style="${style}">${video}<section class="profile-card ${esc(p.entrance_animation)}">${avatar}<h1>${esc(p.display_name || user.username)}</h1><p>${esc(p.bio)}</p><div class="socials">${socialHtml}</div><div class="profile-links">${linkHtml}</div>${audio}${report}</section></main>`, viewer);
}
